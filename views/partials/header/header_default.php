	...
	<nav
		...
		<div class="navbar-utils">
			...
			if ( class_exists( 'Tutor\Ecommerce\CartController' ) && 'tutor' == tutor_utils()->get_option( 'monetize_by' ) && 'header_fullwidth_center' !== get_theme_mod( 'header_type_select' ) ) {
				$nativeCartController = new CartController(); ?>
				<?php
				if ( true === get_theme_mod( 'cart_btn_toggle', true ) ) {
					$items = $nativeCartController->get_cart_items()['courses'];
					if ( $items ) { ?>
						<a class="cart-contents" href="<?php echo esc_url( $nativeCartController->get_page_url() ); ?>"
							...
							<span class="tutor_native_cart_count">(<?php echo $nativeCartController->get_cart_items()['courses']['total_count']; ?>)</span>
							</span>
						</a>
					<?php 
					}
